...
public class UserController {
    ...
    @PutMapping("/updateMembership/{userId}/{isMember}")
    public ResponseEntity<String> updateMembership(@PathVariable String userId, @PathVariable boolean isMember) {
        User user = userService.getUserById(userId);
        if (user == null) {
            return ResponseEntity.status(400).body("User not found");
        }
        user.setMember(isMember);
        return ResponseEntity.status(200).body("Membership status updated successfully");
    }
    ...
    public ResponseEntity<ArrayList<Product>> getPurchases() {
        return ResponseEntity.status(200).body(userService.getPurchasedProducts());
    }
    ...
    //---------------------------------
    //extra
    @PutMapping("/changUsername/{userId}/{oldName}/{newName}")
    public ResponseEntity changUsername(@PathVariable String userId,@PathVariable String oldName,String newName){
        boolean isChanged=userService.changeUsername(userId, oldName, newName);
        if(isChanged){
